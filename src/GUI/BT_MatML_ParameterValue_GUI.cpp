...
)
{
	...
	wxScrolledWindow* ParameterValuePanel = new wxScrolledWindow(ParameterValueNotebook, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxHSCROLL | wxVSCROLL);
...
void ParameterValue_GUI_Base::Update( ParameterValue* element, const boost::shared_ptr<MatML_Doc> doc)
{
	wxString str;
	...
	str.clear();
	str << _std2wx(::string(DataFormat::_xsd_DataFormat_literals_[int(element->format())]));
...
wxTreeItemId ParameterValue_GUI_Base::SetupMatMLTreeCtrl(TreeCtrlSorted*& MatMLTreeCtrl, 
...
)
{
	...
	Data_GUI::SetupMatMLTreeCtrl(MatMLTreeCtrl, CurrentId, Element.Data(), wxTreeItemId());

	//Setup Element Uncertainty
	{
		ParameterValue::Uncertainty_sequence& cont(Element.Uncertainty());
		ParameterValue::Uncertainty_iterator iter(cont.begin());
		if (!cont.empty())
			for (; iter != cont.end(); ++iter)
	...
	{
		ParameterValue::Qualifier_sequence& cont(Element.Qualifier());
		ParameterValue::Qualifier_iterator iter(cont.begin());
		if (!cont.empty())
			for (; iter != cont.end(); ++iter)
	...
	//Setup Element Notes
	{
		ParameterValue::Notes_optional& cont(Element.Notes());
		if (cont.present())
...
void ParameterValue_GUI::OnParameterValueParameterChoice(wxCommandEvent& event)
{
	...
	try {
		...
		if (Element != 0) {
			...
			Element->parameter() = NewData;

			//Set up label string

			::std::string label("ParameterValue-");

			xml_schema::idref& idref(Element->parameter());//get parameter for label
			const ParameterDetails* pram = dynamic_cast<const ParameterDetails*>(&*idref);
