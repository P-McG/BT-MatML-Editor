...
wxTextCtrl* Formula_GUI_Base::GetFormulaTextCtrl()
{
	return m_FormulaTextCtrl;
}
...
wxNotebook* Formula_GUI_Base::Create(wxWindow* parent, wxTextCtrl*& FormulaTextCtrl)
{
	...
	wxWindow* FormulaPanel(CreateFormulaPanel(FormulaNotebook, FormulaTextCtrl));
...
wxWindow* Formula_GUI_Base::CreateFormulaPanel(wxWindow* parent, wxTextCtrl*& FormulaTextCtrl)
{
	wxScrolledWindow* FormulaPanel = new wxScrolledWindow(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxHSCROLL | wxVSCROLL);
...
void Formula_GUI_Base::Update( const observer_ptr<Formula> element)
{
	if (element) {
		wxString str;
...
wxString Formula_GUI_Base::GetTreeLabel(const observer_ptr<Formula> element)
{
	wxString str;
	str << wxT("Formula");
...
TreeCtrlSorted* Formula_GUI::GetMatMLTreeCtrl()
{
...
void Formula_GUI::OnFormulaTextCtrl(wxCommandEvent& event)
{
	...
	try {
		...
		if (Element != 0) {
			wxString str(GetFormulaTextCtrl()->GetValue());
