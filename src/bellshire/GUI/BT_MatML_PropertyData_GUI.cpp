...
wxChoice* PropertyData_GUI_Base::GetPropertyDataPropertiesChoice()
{
	return m_PropertyDataPropertiesChoice;
}

wxChoice* PropertyData_GUI_Base::GetPropertyDataTechniqueChoice()
{
	return m_PropertyDataTechniqueChoice;
}

wxChoice* PropertyData_GUI_Base::GetPropertyDataSourceChoice()
{
	return m_PropertyDataSourceChoice;
}

wxChoice* PropertyData_GUI_Base::GetPropertyDataSpecimenChoice()
{
	return m_PropertyDataSpecimenChoice;
}

wxChoice* PropertyData_GUI_Base::GetPropertyDataTestChoice()
{
	return m_PropertyDataTestChoice;
}

wxTextCtrl* PropertyData_GUI_Base::GetPropertyDataDelimiterTextCtrl()
{
	return m_PropertyDataDelimiterTextCtrl;
}

wxTextCtrl* PropertyData_GUI_Base::GetPropertyDataQuoteTextCtrl()
{
	return m_PropertyDataQuoteTextCtrl;
}

PropertyDetails_Display_GUI* PropertyData_GUI_Base::GetPropertyDataPropertyDetails_Display_GUI()
{
	return m_PropertyDataPropertyDetails_Display_GUI;
}

MeasurementTechniqueDetails_Display_GUI* PropertyData_GUI_Base::GetPropertyDataMeasurementTechniqueDetails_Display_GUI()
{
	return m_PropertyDataMeasurementTechniqueDetails_Display_GUI;
}

DataSourceDetails_Display_GUI* PropertyData_GUI_Base::GetPropertyDataDataSourceDetails_Display_GUI()
{
	return m_PropertyDataDataSourceDetails_Display_GUI;
}

SpecimenDetails_Display_GUI* PropertyData_GUI_Base::GetPropertyDataSpecimenDetails_Display_GUI()
{
	return m_PropertyDataSpecimenDetails_Display_GUI;
}

TestConditionDetails_Display_GUI* PropertyData_GUI_Base::GetPropertyDataTestConditionDetails_Display_GUI()
{
	return m_PropertyDataTestConditionDetails_Display_GUI;
}
...
)
{
	...
	wxWindow* PropertyDataPanel(CreatePropertyDataPanel(PropertyDataNotebook,
		PropertyDataPropertiesChoice,
		PropertyDataTechniqueChoice,
		PropertyDataSourceChoice,
		PropertyDataSpecimenChoice,
		PropertyDataTestChoice,
		PropertyDataDelimiterTextCtrl,
		PropertyDataQuoteTextCtrl,
		PropertyDataPropertyDetails_Display_GUI,
		PropertyDataMeasurementTechniqueDetails_Display_GUI,
		PropertyDataDataSourceDetails_Display_GUI,
		PropertyDataSpecimenDetails_Display_GUI,
		PropertyDataTestConditionDetails_Display_GUI
	));
...
wxWindow* PropertyData_GUI_Base::CreatePropertyDataPanel(wxWindow* parent,
	wxChoice*& PropertyDataPropertiesChoice,
	wxChoice*& PropertyDataTechniqueChoice,
	wxChoice*& PropertyDataSourceChoice,
	wxChoice*& PropertyDataSpecimenChoice,
...
)
{
	...
	wxScrolledWindow* PropertyDataPanel = new wxScrolledWindow(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxALWAYS_SHOW_SB | wxHSCROLL | wxVSCROLL);
	...
	return PropertyDataPanel;
}

void PropertyData_GUI_Base::Update( const observer_ptr<PropertyData> element, const observer_ptr<MatML_Doc> doc)
{
...
wxString PropertyData_GUI_Base::GetTreeLabel(const observer_ptr<PropertyData> element)
{
	wxString str;
...
void PropertyData_GUI::OnPropertyDataPropertiesChoice(wxCommandEvent& event)
{
	...
	try {
		auto element_observer{ boost::any_cast<observer_ptr<PropertyData>>(item->GetAnyMatMLDataPointer())};
		if (element_observer) {

			//Get wxWidget info and Store MatML Data
			PropertyData::property_type NewData(LabelToId(_wx2std(GetPropertyDataPropertiesChoice()->GetStringSelection())).c_str());
			element_observer->property() = NewData;

			//Set up label string

			::std::string label("PropertyData-");

			xml_schema::idref& idref(element_observer->property());//get parameter for label
			const PropertyDetails* prop = dynamic_cast<const PropertyDetails*>(&*idref);
			...
			GetPropertyDataPropertyDetails_Display_GUI()->Update(element_observer);
...
void PropertyData_GUI::OnPropertyDataTechniqueChoice(wxCommandEvent& event)
{
	...
	try {
		auto element{ boost::any_cast<observer_ptr<PropertyData>>(item->GetAnyMatMLDataPointer())};
		if (element) {

			//Get wxWidget info and Store MatML Data
			::std::string id(LabelToId(_wx2std(GetPropertyDataTechniqueChoice()->GetStringSelection())));
			...
			GetPropertyDataMeasurementTechniqueDetails_Display_GUI()->Update(element);
		}
	}
	catch (const boost::bad_any_cast&) { return; }
}
...
void PropertyData_GUI::OnPropertyDataSourceChoice(wxCommandEvent& event)
{
	...
	try {
		...
		if (element_observer) {
			...
			//Get wxWidget info and Store MatML Data
			::std::string id(LabelToId(_wx2std(GetPropertyDataSourceChoice()->GetStringSelection())));
			if (id.empty()) element_observer->source().reset();//Empty data is still data so reset.
			else {
				PropertyData::source_type NewData(id.c_str());
				element_observer->source() = NewData;
			}

			GetPropertyDataDataSourceDetails_Display_GUI()->Update(element_observer);
		}
	}
	catch (const boost::bad_any_cast&) { return; }
}
...
void PropertyData_GUI::OnPropertyDataSpecimenChoice(wxCommandEvent& event)
{
	...
	try {
		...
		if (element_observer) {
			...
			//Get wxWidget info and Store MatML Data
			::std::string id(LabelToId(_wx2std(GetPropertyDataSpecimenChoice()->GetStringSelection())));
			if (id.empty()) 
				element_observer->specimen().reset();//Empty data is still data so reset.
			else {
				PropertyData::specimen_type NewData(id.c_str());
				element_observer->specimen() = NewData;
			}

			//Update the SpecimenDetails Data on the panel
			GetPropertyDataSpecimenDetails_Display_GUI()->Update(element_observer);
		}
	}
	catch (const boost::bad_any_cast&) { return; }
}
...
void PropertyData_GUI::OnPropertyDataTestChoice(wxCommandEvent& event)
{
	...
	try {
		...
		if (element_observer) {
			...
			//Get wxWidget info and Store MatML Data
			::std::string id(LabelToId(_wx2std(GetPropertyDataTestChoice()->GetStringSelection())));
			if (id.empty()) 
				element_observer->test().reset();//Empty data is still data so reset.
			else {
				PropertyData::test_type NewData(id.c_str());
				element_observer->test() = NewData;
			}

			//Update the TestConditionDetails Data on the panel
			GetPropertyDataTestConditionDetails_Display_GUI()->Update(element_observer);
		}
	}
	catch (const boost::bad_any_cast&) { return; }
}
...
void PropertyData_GUI::OnPropertyDataDelimiterTextCtrl(wxCommandEvent& event)
{
	...
	try {
		auto element_observer{ boost::any_cast<observer_ptr<PropertyData>>(item->GetAnyMatMLDataPointer()) };
		if (element_observer) {
			wxString str(GetPropertyDataDelimiterTextCtrl()->GetValue());
			PropertyData::delimiter_type NewData((const char*)str.mb_str(wxConvUTF8));
			(element_observer->delimiter()) = NewData;
			return;
		}
	}
	catch (const boost::bad_any_cast&) { return; }
...
void PropertyData_GUI::OnPropertyDataQuoteTextCtrl(wxCommandEvent& event)
{
	...
	try {
		 auto element_observer{ boost::any_cast<observer_ptr<PropertyData>>(item->GetAnyMatMLDataPointer()) };
		if (element_observer) {
			PropertyData::quote_optional& cont(element_observer->quote());
			wxString str(GetPropertyDataQuoteTextCtrl()->GetValue());
