...
MatML_Doc_GUI_Base::MatML_Doc_GUI_Base( wxWindow* parent )
{
	/// Create Gui
	m_GUI = Create(parent);
}
...
wxNotebook* MatML_Doc_GUI_Base::Create(wxWindow* parent)
{
	...
	wxWindow* MatML_DocPanel(CreateMatML_DocPanel(MatML_DocNotebook));
...
wxWindow* MatML_Doc_GUI_Base::CreateMatML_DocPanel(wxWindow* parent)
{
	/// Creates the MatML data panel
	wxScrolledWindow* MatML_DocPanel = new wxScrolledWindow(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxHSCROLL | wxVSCROLL);
...
wxString MatML_Doc_GUI_Base::GetTreeLabel(const observer_ptr<MatML_Doc> element)
{
	wxString str;

	str << _std2wx(std::string("MatML_Doc"));
...
TreeCtrlSorted* bellshire::GUI::MatML_Doc_GUI::GetMatMLTreeCtrl()
{
	return m_MatMLTreeCtrl;
}

observer_ptr<MatML_Doc> bellshire::GUI::MatML_Doc_GUI::GetMatMLDoc()
{
	return m_MatMLDoc;
}
...
void MatML_Doc_GUI::SetEvtHandlerVar(TreeCtrlSorted*& MatMLTreeCtrl, observer_ptr<MatML_Doc>& MatMLDoc)
{
...
/// </summary>
/// <param name="MatMLTreeCtrl"></param>
void MatML_Doc_GUI::SetMatMLDoc(const observer_ptr<MatML_Doc>& matml_doc)
{
	if(matml_doc)
...
void MatML_Doc_GUI::OnDeleteMaterial(wxCommandEvent& event) {
	...
	try {
		auto material_shared = boost::any_cast<observer_ptr<Material> >(item->GetAnyMatMLDataPointer());
		if (material_shared) {
			if (material_shared->id().present()) {
							...
							if (!compiter->Class().empty()) {
								...
								do {
									...
									if (!classiter->ParentMaterial().empty()) {
										const auto& parentmaterial(observer_ptr<Material>(dynamic_cast<Material*>(&*classiter->ParentMaterial().front().id())));
										if (material_shared == parentmaterial) erase = true;
									}
									if (erase) {
			...
			do {
				observer_ptr<Material>iter_shared(&*iter);
				if (iter_shared == material_shared) {
					...
					if (update == true) {
						...
						MatML_Doc_GUI::TraverseMatMLTree(m_MatMLDoc, func1);
						//
						if (iter != cont.end()) {
							wxTreeItemId newtreeitem(MatMLtoTreeCtrl(m_MatMLTreeCtrl, iter_shared));
