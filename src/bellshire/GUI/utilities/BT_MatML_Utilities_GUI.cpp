...
wxTreeItemId Utilities::MatMLtoTreeCtrl(TreeCtrlSorted* MatMLTreeCtrl, observer_ptr<Material> material)
{
	...
	do {
		...
		try {
			const auto TreeMaterial = boost::any_cast<observer_ptr<Material>>(MaterialItem->GetAnyMatMLDataPointer());
...
std::list< std::pair< observer_ptr<Material>, xml_schema::string > > 
Utilities::GetMaterialIDPair(observer_ptr<MatML_Doc> MatMLDoc, std::list<xml_schema::string> ClassFilters)
{
	std::list< std::pair< observer_ptr<Material>, xml_schema::string > > MaterialIDPair;
		...
		if (!cont.empty()) {
			...
			for (; iter != cont.end(); ++iter) {
				if (iter->id().present()) {
					...
					if (!ClassFilters.empty()) {
						...
						for (std::list<xml_schema::string>::iterator iterClassFilters(ClassFilters.begin()); iterClassFilters != ClassFilters.end(); ++iterClassFilters)
						{
							if (!iterClassFilters->empty()) {
								bool inFilter = InFilter(material, &*iterClassFilters);
					...
					if (addToList) {
						std::pair< observer_ptr<Material>, xml_schema::string> pair(material, iter->id().get());
...
/// <param name="ClassFilters"></param>
/// <returns></returns>
bool Utilities::InFilter(observer_ptr<Material> material, std::list<xml_schema::string> Filter)
{
...
/// <returns></returns>
bool Utilities::InFilter(observer_ptr<Material> material, xml_schema::string* Filter)
{
...
/// <param name="MatMLDoc"></param>
/// <returns></returns>
std::list<Name*> Utilities::GetMatMLDocClassNames(observer_ptr<MatML_Doc> matml_doc)
{
	...
	if (matml_doc) {
		...
		MatML_Doc::Material_sequence& cont(matml_doc->Material());
		MatML_Doc::Material_iterator iter(cont.begin());
		if (!cont.empty()) {
...
/// <param name="m"></param>
/// <param name="ms"></param>
void Utilities::CollectBranchMaterials(observer_ptr<MatML_Doc> doc, observer_ptr<Material> m, std::vector<observer_ptr<Material>>& ms)
{
...
/// <param name="m"></param>
/// <param name="ms"></param>
void Utilities::CollectBranchMaterialsBackwards(observer_ptr<Material> m, std::vector<observer_ptr<Material>>& ms)
{
	if (m) {
		if (!m->ComponentDetails().empty()) {
			...
			for (Material::ComponentDetails_iterator ci = cs.begin(); ci != cs.end(); ci++) {
				...
				for (ComponentDetails::Class_iterator classiter = classcont.begin(); classiter != classcont.end(); ++classiter) {
					...
					observer_ptr<Material> parentmaterial{ static_cast<Material*>(&*classiter->ParentMaterial().front().id()) };
					if (parentmaterial == NULL || parentmaterial == m) continue;

					//check for duplicates
					bool dupl = false;
					for (std::vector<observer_ptr<Material>>::iterator mi = ms.begin(); mi != ms.end(); mi++) {//duplicate check loop
...
/// <param name="m"></param>
/// <param name="ms"></param>
void Utilities::CollectBranchMaterialsForewards(observer_ptr<MatML_Doc> doc, observer_ptr<Material> m, std::vector<observer_ptr<Material>>& ms)
{
			...
			if (!docmi->ComponentDetails().empty()) {
				...
				for (Material::ComponentDetails_iterator ci = cs.begin(); ci != cs.end(); ci++) {
					...
					for (ComponentDetails::Class_iterator classiter = classcont.begin(); classiter != classcont.end(); ++classiter) {
						...
						auto parentmaterial(observer_ptr<Material>{static_cast<Material*>(&*classiter->ParentMaterial().front().id())});
						...
						for (std::vector<observer_ptr<Material>>::iterator mi = ms.begin(); mi != ms.end(); mi++) {//duplicate check loop
							...
							if (*mi==observer_ptr<Material>(&*docmi)) {//see if they have the same memory location
								dupl = true;
								break;//get out of duplicate check loop
							}
						}
