...
wxNotebook* Element_GUI_Base::Create(wxWindow* parent)
{
	...
	wxWindow* ElementPanel(CreateElementPanel(ElementNotebook));
...
wxWindow* Element_GUI_Base::CreateElementPanel(wxWindow* parent)
{
	wxScrolledWindow* ElementPanel = new wxScrolledWindow(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxHSCROLL | wxVSCROLL);
...
void Element_GUI_Base::Update( const observer_ptr<Element> element)
{
	wxString str;
...
wxString Element_GUI_Base::GetTreeLabel(const observer_ptr<Element> element)
{
	  wxString str;
	  str << wxT("Element");
...
TreeCtrlSorted* Element_GUI::GetMatMLTreeCtrl()
{
	return m_MatMLTreeCtrl;
}

::boost::any Element_GUI::GetMatMLItemToCopy()
{
