...
wxNotebook* AssociationDetails_GUI_Base::Create(wxWindow* parent)
{
	...
	wxWindow* AssociationDetailsPanel(CreateAssociationDetailsPanel(AssociationDetailsNotebook));
...
wxWindow* AssociationDetails_GUI_Base::CreateAssociationDetailsPanel(wxWindow* parent
	)
{
	wxScrolledWindow* AssociationDetailsPanel = new wxScrolledWindow(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxHSCROLL | wxVSCROLL);
...
void AssociationDetails_GUI_Base::Update( const observer_ptr<AssociationDetails> element)
{
	//Nothing to update
...
wxString AssociationDetails_GUI_Base::GetTreeLabel(const observer_ptr<AssociationDetails> Element)
{
	wxString str;
...
TreeCtrlSorted* AssociationDetails_GUI::GetMatMLTreeCtrl()
{
	return m_MatMLTreeCtrl;
}

::boost::any AssociationDetails_GUI::GetMatMLItemToCopy()
{
...
void AssociationDetails_GUI::OnBumpDown(wxCommandEvent& event)
{
	...
	try {
		if (anyptrparent.type() == typeid(ComponentDetails*)) {
			...
			auto element{ remove_strongly_typed_on_observer_ptr_v(boost::any_cast<observer_ptr<AssociationDetails> > (anyptr)) };
			auto parent_element{ remove_strongly_typed_on_observer_ptr_v(boost::any_cast<observer_ptr<ComponentDetails> > (anyptr)) };

			if (parent_element) {
				auto& cont{ parent_element->AssociationDetails() };
				auto data{ MatML_Base::FindAndBumpDown(element, cont) };

				MatMLTreeCtrlBumpDown<AssociationDetails_GUI>(m_MatMLTreeCtrl, itemParentId, itemId, data.first, nextitemId, data.second);
			}
		}
	}
	...
	try {
		if (anyptrparent.type() == typeid(ComponentDetails*)) {
			...
			if (parent_element) {
				...
				MatMLTreeCtrlBumpUp<AssociationDetails_GUI>(m_MatMLTreeCtrl, itemParentId, previtemId, data.first, itemId, data.second);
			}
		}
	}
	catch (const boost::bad_any_cast&) {};//do nothing
